...
//!
//! Same ping-pong scheme as `rd`: two Rgba32Float state textures hold (u, v).
//! Each frame reseeds on request, then runs `substeps` × cs_step, then
//! colorizes the front buffer into `display_view` for blit.

use bytemuck::{Pod, Zeroable};

pub const FHN_N: u32 = 256;
...
impl FhnRenderer {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            label: Some("fhn-shader"),
            source: wgpu::ShaderSource::Wgsl(include_str!("../shaders/fhn.wgsl").into()),
        });
        ...
        let tex_a = make_state("fhn-state-a");
        let tex_b = make_state("fhn-state-b");
        ...
        let display_uniform = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("fhn-display-uniform"),
        ...
        queue.write_buffer(
            &display_uniform,
            0,
            bytemuck::bytes_of(&DispU { n: FHN_N, _p0: 0, _p1: 0, _p2: 0 }),
        );

        let display_bg_from_a = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("fhn-display-from-a"),
            layout: &display_bgl,
            entries: &[
        ...
        let display_layout = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
            label: Some("fhn-display-layout"),
            bind_group_layouts: &[&display_bgl],
            push_constant_ranges: &[],
        });

        let display_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("fhn-display-pipeline"),
